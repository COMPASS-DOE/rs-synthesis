...
\
\
The relationship is intuitive, the stronger the decrease in precipitation, the stronger the decrease in soil respiration (Figure 6A). No such relationship was foudn in increased precipitation studies (Figure 6B).
\
\
```{r}
...
\
The organic C content of soil was highly significant for both manipulation types (p < 0.001 for -P, < 0.01 for +P). The overall effect of soil organic C was positive, meaning that soils with more C had higher respiration rates in response to treatments. However, when effect sizes are plotted against orgnaic C stocks (Figure 7), we found that soils with the highest organic contetn were the most resistant to changes in precipitation. 
\

```{r Soil_grid_ocs, echo = FALSE, warning = FALSE}
...
\
\
Clay had low predictor importance for IP, high for DP. The linear regression between effect sizes and clay content supports this result (Figure 8). However, the effect was not significant in the deceased precipitation model, and clay content was not of sufficient importance to include in the increased precipitation model.
\
\
```{r Soil_grid_clay, echo = FALSE, warning = FALSE}
