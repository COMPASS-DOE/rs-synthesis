...
meta_control <- cont1
meta_control$Control_SD <- cont2$Control_SD
...
meta_manip <- manip1
meta_manip$Manip_SD <- manip2$Manip_SD

# ...and join with the manipulation data
meta_manip %>% 
  left_join(meta_control, 
            by = c("Study_number", "Study_midyear", "Ecosystem_type",
                   "Latitude", "Meas_method", "Soil_type", "Soil_drainage",                   "Elevation", "depvar")) %>% 
  rename("Variable" = "depvar", "Quality_flag" = "Quality_flag.x") %>% 
  select(-Quality_flag.y) ->
  meta_df

# At this point:
# 1. meta_df should have no duplicate rows
# 2. All study numbers in dat_rs should be present in meta_df
#
...
meta_df %>%
  ...
  mutate(YearOfStudy = Study_midyear - min(Study_midyear)) -> meta_time

meta_time %>%
  group_by(Study_number, Ecosystem_type) %>%
  mutate(PC_group = as.numeric(as.factor(Percent_control))) -> meta_timePC
...
drainage_type %>%
  mutate(Ecosystem_type = trimws(Ecosystem_type)) %>%
  group_by(Ecosystem_type) %>%
  summarise(n_studies = length(unique(Study_number)), .groups = "drop") %>%
