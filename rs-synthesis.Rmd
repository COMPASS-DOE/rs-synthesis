...
# First map: good for QA/QC. Shows terrain and vegetation and points are colored by ecosystem type
smap <- get_stamenmap(bbox = c(left = -180, bottom = -50, right = 179.9999, top = 80), zoom = 3)

ggmap(smap) +
  geom_point(data = meta_coords, aes(x = Longitude, y = Latitude, color = Ecosystem_type)) +
  theme_void()

#OR

# Second map: good for spatial distribution. Outline map with points in red
...
ggplot() +
  geom_map(
    ...
    color = "darkgrey", fill = "white", size = 0.1) + 
  geom_point(data = meta_coords, aes(x = Longitude, y = Latitude), color = "red", alpha = 0.5) +
  theme_void() +
