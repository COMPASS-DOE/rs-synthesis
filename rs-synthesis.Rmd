...
ggplot(test, aes(average, Duration, label = count)) + 
  ...
  ggtitle("Number of Studies by Ecosystem and Study Duration v1")

ggplot(metadat) +
  geom_histogram(aes(Duration, fill=Ecosystem_type), position = "stack", bins = 11, col="black") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
  facet_grid(Manipulation~.) + ggtitle("Number of Studies by Ecosystem and Study Duration v2")
...
ggplot(Coef_drought, aes(et_intercept,
                         row.names(Coef_drought),
                         na.rm = TRUE)) + 
  geom_vline(xintercept = 0) +
  geom_errorbarh(aes(xmax = et_intercept + se,
                     xmin = et_intercept - se,
                     na.rm = TRUE)) +
  geom_point(size = 4, na.rm = TRUE)

ggplot(Coef_irrigation, aes(et_intercept,
