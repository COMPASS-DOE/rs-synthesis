...
library(dmetar)  # devtools::install_github("MathiasHarrer/dmetar"), skip updates
library(broom)
library(bookdown)
library(ggmap)
...
read_csv("./rs/Rs Studies 2018+ - Water Manipulation.csv",
         col_types = "cdddcdcccdcdccdddddddddddc"
) %>%
  select(
    Study_number, Record_number, Author, Study_midyear, Latitude, Ecosystem_type,
    Manipulation, Manipulation_level, Meas_interval, Meas_method, Soil_type,
...
## Effect Sizes within Ecosystems
...
EffectSizePairs %>%
  full_join(Ecosystem_studies, by = "Ecosystem_type") -> EES_table
...
#Soil grids vs study reported soil organic carbon
...
\
\
Comparison of SoilGrids predicted soil organic carbon and soil clay contents, relative to study-reported values.  
\
\
...
text(0.07, rev(seq(3:1)), Vari_ss)
op <- par(cex=1, font=2)
text(-.089, 4.2, "Dependent Variable")
text(.07, 4.2, "Sample Size")
...
text(-.29, rev(seq(3:1)), Vard_ss)
op <- par(cex=1, font=2)
text(-.515, 4.2, "Dependent Variable")
text(-.29, 4.2, "Sample Size")
text(.2, 4.2, "ln(RR) [95% CI]")
...
mmi_irrigation1 <- multimodel.inference(
  TE = "yi", seTE = "vi",
  data = metadat[metadat$Manipulation == "Irrigation",],
  predictors = c("Ecosystem_type", "Duration", "Percent_control",
                 "Meas_interval", "sg_ocs", "sg_clay"),
  interaction = TRUE
)
```

```{r print mmi_irrigation, include = FALSE, cache=TRUE}
#print(mmi_irrigation$top5.models)
print(mmi_irrigation1$predictor.importance.plot)
# reduce to top four to allow interactions
mmi_irrigation2 <- multimodel.inference(
  TE = "yi", seTE = "vi",
  data = metadat[metadat$Manipulation == "Irrigation",],
  predictors = c("sg_ocs", "Meas_interval", "Ecosystem_type", "Duration"),
  interaction = TRUE
)
print(mmi_irrigation2$predictor.importance.plot)
# soil organic carbon, ecosystem, and duration (as well as ecosystem x duration) and measurement interval are all highly important
# due to high levels of confounding between measurement interval and both
# ecosystem type and study duration (see alternateModels.R code),
# it is dropped from the model below

```
\
\
For +P studies, the primary effect of SOC, ecosystem type, study duration, and the interaction between ecosystem type and duration were the four highest ranked predictors. In contrast, the -P model ranked ecosystem type, SOC, clay, and duration as the top four predictors.
\
\
...
metadat %>%
  ...
  select("Study_number", "Ecosystem_type",
         "Percent_control", "sg_clay",
         "Duration", "sg_ocs", "yi", "vi") %>%
  na.omit() -> metadat_i

# rma.mv conducts a meta-analysis multivariate mixed effects model
# ie., meta-regression
# the - 1 gives us individual estimates for ecosystem types (fixed effects)
...
text(-30, 6.25, "Sample Size")
text(120, 6.25, "Mean [95% CI]")
...
mmi_drought <- multimodel.inference(
  TE = "yi", seTE = "vi",
  data = metadat[metadat$Manipulation == "Drought", ],
  predictors = c("Ecosystem_type", "sg_ocs", "sg_clay",
                 "Meas_interval", "Percent_control", "Duration"),
  interaction = TRUE
)
```

```{r print mmi_drought, include=FALSE, cache=TRUE}
print(mmi_drought$predictor.importance.plot)
# ecosystem, soil organic carbon, clay, and percent_control are the top four
# even when allowing for interactions (see below)
mmi_drought2 <- multimodel.inference(
  TE = "yi", seTE = "vi",
  data = metadat[metadat$Manipulation == "Drought", ],
  predictors = c("Ecosystem_type", "sg_ocs", "sg_clay", "Percent_control"),
  interaction = TRUE
)
print(mmi_drought2$predictor.importance.plot)
```
...
text(-110, rev(seq(6:1)), Ecod_ss)
op <- par(cex = 1, font = 2)
text(-180, 7.3, "Ecosystem")
text(-110, 7.3, "Sample Size")
