...
srdb %>% 
  # Studies with manipulations that start with "Irrigat"
...
srdb %>% 
  filter(grepl("^Precip[a-z ]*$", Manipulation, ignore.case = TRUE)) %>%
  distinct(Study_number) %>% 
  left_join(srdb, by = "Study_number") %>%
  group_by(Study_number) %>% 
  filter(Manipulation == "None" | 
           grepl("^Precip[a-z ]*$", Manipulation, ignore.case = TRUE)) -> precipitation
...
kable(precip_summary) %>% kable_styling(full_width = FALSE)
