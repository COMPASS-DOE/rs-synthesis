...
#### Water manipulations: irrigation and drought
...
srdb %>% 
  ...
  filter(grepl("^(Irrigat[a-z]*|Drought|Precipitation amount change)$", Manipulation, ignore.case = TRUE)) %>% 
  ...
  filter(Manipulation == "None" |
           grepl("^(Irrigat[a-z]*|Drought|Precipitation amount change)$", Manipulation, ignore.case = TRUE))  -> water_manipulations
...
# Make table display
kable(irr_summary) %>% kable_styling(full_width = FALSE)
...
water_manipulations %>% 
  ...
  left_join(water_manipulations, by = c("Study_number", "Ecosystem_type", "Study_midyear")) %>% 
  mutate(rr = log(Rs_annual / Rs_none), 
         Study = paste(Study_number, Author)) %>% 
  filter(Manipulation != "None") -> wm_response_ratios

  ggplot(wm_response_ratios, aes(x = rr, y = fct_reorder(Study, rr))) + 
...
kable(elevat_summary) %>% kable_styling(full_width = FALSE)
...
bind_rows(wm_summary, drain_summary, elevat_summary) %>% 
