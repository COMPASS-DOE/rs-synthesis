...
if (file.exists(clayfile) && file.exists(ocsfile)) {
  ...
  # geoTIFF files are not present; issue a warning and use fake (random) data
  warning("geoTIFF files are not present! Analysis will not be correct")
  meta_df$sg_clay <- runif(nrow(meta_df))
  meta_df$sg_ocs <- runif(nrow(meta_df))
}
```
...
op <- par(cex = 1, font = 2)
text(-220, 6.25, "Ecosystem")
text(-80, 6.25, "Sample Size")
text(350, 6.25, "Mean [95% CI]")
...
op <- par(cex = 1, font = 2)
text(-235, 7.3, "Ecosystem")
text(-120, 7.3, "Sample Size")
text(160, 7.3, "Mean [95% CI]")
...
## Decreased Precipitation
```{r Intensity_effect_-P, fig.cap = " Linear regression of manipulation intensity and effect size for decreased precipitaiton studies.", message = FALSE, warning = FALSE}
...
```{r Intensity_effect_+P, fig.cap = " Linear regression of manipulation intensity and effect size for increased precipitaiton studies.", message = FALSE, warning = FALSE}
...
\
\
The intensity of the manipulation ranked with higher importance for -P studies (Figure \autoref(fig:Intensity_effect_-P) than +P (Figure \autoref(fig:Intensity_effect_+P). The relationship is intuitive: the stronger the decrease in precipitation, the stronger the decrease in soil respiration (Figure \autoref(fig:Intensity_effect_-P) . While manipulation intensity was not included in the primary model for +P (due to being below the top 4 ranking threshold), If the effect of intensity was included in the +P model, a positive relationship exists (Figure \autoref(fig:Intensity_effect_+P), predominantly driven by one very high treatment in a savanna. The slope of this regression line was however still lower than that found in -P.  
\
\
