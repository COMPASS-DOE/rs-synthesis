...
dat %>% 
  ...
  select(-Record_number, -Author, -Manipulation, -Manipulation_level, -N, -SM_mean, -SM_sd, -Notes)->
  dat_control
dat_control %>% 
...
ggplot(dat_rs, aes(control, manip, color = Manipulation)) +
  geom_point(size = 4, na.rm = TRUE) + geom_abline() + facet_wrap(~depvar, scales = "free")
